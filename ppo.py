...
def ppo(trajectories, policy, critic, n_timesteps, policy_optimizer, vf_optimizer):
    ...
    for _ in range(epochs):
        ...
        for i in range(0, n_timesteps, batch_size):
            ...
            action_dist = torch.distributions.Categorical(policy(batch_observations))
            ...
            vf_loss = torch.mean((vf - batch_value_targets)**2)
            
            c1 = 0.5
            c2 = 0.01
            
            total_loss = clipped_surrogate_objective + c1 * vf_loss - c2 * entropy  
